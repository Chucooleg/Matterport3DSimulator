...
class OutputData(object):
    ...
    def compute_entropy_per_datapt(cls, datapt):
        ...
        Returns:
            1 numpy array for navigation entropy of shape (num timestep,)
        """
        nav_ent_initial_list = [cls.compute_entropy(time_step) for time_step in datapt['agent_nav_logits']]
        return nav_ent_initial_list
    ...
    def compute_softmax_per_datapt(cls, datapt):
        ...
        Returns:
            1 numpy array for navigation softmax of shape (num timestep, num decision classes)
        """
        nav_softmax_initial_list = [cls.compute_softmax(time_step) for time_step in datapt['agent_nav_logits']]
        return nav_softmax_initial_list

    @classmethod
    def compute_cross_entropy_per_datapt(cls, datapt):
        num_decisions = len(datapt['trajectory']) - 1
        nav_cross_ent_initial_list = [
            cls.compute_cross_entropy(datapt['agent_nav_logits'][i], datapt['teacher_nav'][i]) for i in
            range(num_decisions)]
        return nav_cross_ent_initial_list

    def compute_entropy_output_data(self):
        for i in range(len(self.output_data)):
            nav_ent_initial_list = \
                self.compute_entropy_per_datapt(self.output_data[i])
            self.output_data[i]['agent_nav_ent'] = nav_ent_initial_list

    def compute_cross_entropy_output_data(self):
        for i in range(len(self.output_data)):
            nav_cross_ent_initial_list = \
                self.compute_cross_entropy_per_datapt(self.output_data[i])
            self.output_data[i]['agent_nav_cross_ent'] = nav_cross_ent_initial_list

    def compute_softmax_output_data(self):
         for i in range(len(self.output_data)):
            nav_softmax_initial_list = \
                self.compute_softmax_per_datapt(self.output_data[i])
            self.output_data[i]['agent_nav_softmax'] = nav_softmax_initial_list    

    def compute_nav_argmax_output_data(self):
        for i in range(len(self.output_data)):
            timesteps = len(self.output_data[i]['teacher_nav'])
            agent_nav_argmax_initial = []
            for j in range(timesteps):
                agent_nav_argmax_initial.append(np.argmax(self.output_data[i]['agent_nav_logits'][j]))
            self.output_data[i]['agent_nav_argmax'] = agent_nav_argmax_initial
...
class PlotUtils(object):
    ...
    action_reference=None, time_step_specific=False, num_bins=10):
        ...
        for a in range(len(action_reference)):
            ...
            ax.xaxis.set_ticks(np.arange(0.0, 1.0 + bin_width, bin_width*4))
